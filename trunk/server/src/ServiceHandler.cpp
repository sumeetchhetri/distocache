...
ServiceHandler::ServiceHandler(int fd, map<string,string> *params, bool isSSLEnabled, SSL_CTX *ctx, SSLHandler sslHandler) {
	this->fd = fd;
	this->params = params;
	this->isSSLEnabled = isSSLEnabled;
	this->ctx = ctx;
	this->sslHandler = sslHandler;
	logger = Logger::getLogger("ServiceHandler");
}

ServiceHandler::~ServiceHandler() {
}

int ServiceHandler::getLength(string header,int size)
{
...
bool ServiceHandler::validQuery(vector<string> parts, int size, string cmd1, string cmd2)
{
	bool flag = true;
